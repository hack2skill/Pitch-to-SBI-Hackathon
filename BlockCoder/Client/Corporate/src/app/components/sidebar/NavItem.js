...
const NavItem = ({sidebarStatus, menuTitle, subMenu, subMenuArray, hrefLink, children}) => {
    ...
    return (
        <>  
            <Link href={hrefLink}>
               <span  className="inline-flex items-center justify-between py-3 ml-5  hover:text-white-400 hover:bg-gray-700 focus:text-gray-400 focus:bg-gray-700 rounded-lg px-3 cursor-pointer relative group" onClick={subMenuToggle}>
                    {children}
            ...
            {subMenu && (
                <ul className={`${subMenuToggleStatus ? '' : 'hidden'} text-white space-y-2 ml-7`}>
                    {subMenuArray?.map((subMenuSingle, index)=> (
                        ...
                        <li className='cursor-pointer active:text-orange-400 hover:text-gray-500 ml-10  mt-2' >{subMenuSingle.subMenuTitle}</li>
                        </Link>
                        ))
                    }
                </ul>      
